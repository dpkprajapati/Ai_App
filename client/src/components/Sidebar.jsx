...
const Sidebar  =({isMenuOpen, setIsMenuOpen})=>{
    ...
    return (
    <div className={`flex flex-col h-screen min-w-72 p-5 dark:bg-gradient-to-b from-[#242124]/30 to-[#000000]/30 border-r border-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen ? "max-md:-left-full" : "max-md:left-0"}`}>

            {/* logo */}
            <img src={theme==="dark" ? assets.logo_full: assets.logo_full_dark} alt="" className= "w-full max-w-48"/>

             {/* new chat button */}
            <button onClick={createNewChat} className="flex justify-center items-center w-full py-2 mt-10 text-white bg-gradient-to-r from-[#b093fb] to-[#b5576c] text-sm rounded-md cursor-pointer">
            ...
            {/* recent chats  */}
                ...
                <div className="flex-1 overflow-y-scroll mt-3 text-s space-y-3">
                    {
                        chats.filter((chat)=>chat.messages[0]? chat.messages[0]?.content.toLowerCase().includes(search.toLowerCase()): chat.name.toLowerCase().includes(search.toLowerCase())).map((chat)=>(
                            ...
                            key={chat.id} className="p-2 px-4 dark:bg-[#57317C]/10 border border-gray-300 dark:border-[#80609F]/15 rounded-md cursor-pointer flex justify-between group">
                                <div>
                ...
                {/* community images */}
                <div onClick={()=>{navigate("/community"); setIsMenuOpen(false)}}     className="flex intems-center gap-2 p-3 mt-3 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all">
                ...
                <div onClick={()=>{navigate("/credits"); setIsMenuOpen(false)}} className="flex intems-center gap-2 p-3 mt-3 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all">
                ...
                justify-between gap-2 p-3 mt-3 border border-gray-300 dark:border-white/15 rounded-md ">
                    
                    <div className="flex items-center-2 gap-2 text-sm">
                    ...
                    <label className="relative inline-flex cursor-pointer">
                        ...
                        <span className="absolute left-1 top-1 w-3 h-3 bg-white rounded-full ranstion-transfrom peer-checked:translate-x-4"></span>
                    </label>
                </div>  
                {/* user account */}
