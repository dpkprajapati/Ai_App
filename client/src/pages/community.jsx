...
const Community =()=>{
    ...
    const fetchImages = async()=>{
        try{
            ...
            toast.error(error.message || "An error occurred while fetching images")
        }
        setLoading(false)
    }
    ...
    return (
        <div className="p-6 pt-12 xl:px-12 2xl:px-20 w-full mx-auto h-full overflow-y-scroll">
            <h2 className="text-xl font-semibold mb-6 text-gray-800 dark:text-purple-100">community image</h2>
                {images.length > 0 ?(
                    ...
                    <p className="text-center text-gray-600 dark:text-purple-200"> No image Available</p>
                )}
        </div>
    )
}
